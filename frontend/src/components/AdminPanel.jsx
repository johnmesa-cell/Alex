  ...
  return (
    <div className="adm-section fade-up">
      <div className="adm-section-header">
        ...
        <button className="btn-secondary" onClick={fetchData} style={{ fontSize: '0.8rem' }}>↻ Actualizar</button>
      </div>
      ...
      {loading ? <div className="adm-loading"><span className="adm-spinner" />Cargando…</div> : (
        <div className="adm-table-wrap">
          <table className="adm-table">
            <thead>
              <tr><th>#</th><th>Nombre</th><th>Correo</th><th>Rol</th><th>Estado</th><th>Registro</th><th>Último login</th><th>Acción</th></tr>
            </thead>
            <tbody>
              {rows.length === 0
        ...
        <div className="adm-modal-backdrop" onClick={() => setConfirm(null)}>
          <div className="adm-modal adm-modal--sm" onClick={e => e.stopPropagation()}>
            <div className="adm-modal-header">
              ...
              <button className="adm-modal-close" onClick={() => setConfirm(null)}>✕</button>
            </div>
            <p style={{ padding: '1rem 1.5rem' }}>
              ¿Deseas <strong>{confirm.estado === 'activo' ? 'desactivar' : 'activar'}</strong> al usuario <strong>{confirm.nombre}</strong>?
            </p>
            <div style={{ display: 'flex', gap: '0.75rem', padding: '0 1.5rem 1.5rem', justifyContent: 'flex-end' }}>
              ...
              <button
                className={confirm.estado === 'activo' ? 'adm-btn-danger' : 'adm-btn-ok'}
                onClick={() => doToggle(confirm)}
              >
                {confirm.estado === 'activo' ? 'Sí, desactivar' : 'Sí, activar'}
              </button>
            </div>
          </div>
        </div>
              ...
              {rows.length === 0
                ...
                : rows.map(s => (
                  <tr key={s.id_sesion}>
                    ...
                    <td>{fmt(s.ultimaactividad)}</td>
                    <td>{fmt(s.fechaexpiracion)}</td>
          ...
          <table className="adm-table">
            ...
            <tbody>
              {rows.length === 0
                ...
                : rows.map(e => {
                  const accionUpper = (e.accion ?? '').toUpperCase();
                  const color = ACTION_COLOR[accionUpper] ?? 'default';
                  return (
                    <tr key={e.id_evento}>
    ...
    <div className="adm-section fade-up">
      <div className="adm-section-header">
        ...
        <div className="adm-filter-row">
          <select
            value={filtro}
            onChange={e => { setFiltro(e.target.value); load(1, e.target.value); }}
